...
struct ChatManagerConstant {
    ...
    static let notFound = "DIALOG_REMOVED".localized
}
...
class ChatManager: NSObject {
    ...
    func updateStorage() {
        ...
        if QBChat.instance.isConnected == false {
            self.delegate?.chatManager(self, didFailUpdateStorage: "NETWORK_ERROR".localized)
            return
        }
        ...
        loadGroup.notify(queue: DispatchQueue.main) {
            if message.isEmpty {
                self.delegate?.chatManager(self, didUpdateStorage: "COMPLETED".localized)
            } else {
    ...
    func loadUser(_ login: String, completion: ((QBUUser?) -> Void)? = nil) {
        QBRequest.user(withLogin: login, successBlock: { (response, user) in
            self.storage.update(users: [user])
            completion?(user)
        }) { (response) in
            debugPrint("[ChatManager] loadUser with login error: \(self.errorMessage(response: response) ?? "")")
            completion?(nil)
        }
    }
    ...
    func createGroupDialog(withName name: String,
        ...
        QBRequest.createDialog(chatDialog, successBlock: { response, dialog in
            dialog.join(completionBlock: { (error) in
                ...
                self.delegate?.chatManager(self, didUpdateStorage: "CREATE_NEW".localized + dialogName)
    ...
    private func errorMessage(response: QBResponse) -> String? {
        ...
        if response.status.rawValue == 502 {
            errorMessage = "BAD_GATEWAY".localized
