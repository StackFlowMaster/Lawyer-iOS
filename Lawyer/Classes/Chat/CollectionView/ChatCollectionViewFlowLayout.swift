...
class ChatCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    func sizeForItem(at indexPath: IndexPath?) -> CGSize {
        ...
        var height = cellSizeForItem(at: indexPath).height
        height.round(.up)
    ...
    //MARK: - Notifications
    @objc func didReceiveApplicationMemoryWarning(_ notification: NSNotification?) {
        resetLayout()
    }
    ...
    private func cellSizeForItem(at indexPath: IndexPath?) -> CGSize {
        ...
        let containerSize = containerViewSizeForItem(at: indexPath)
        let cellWidth = containerSize.width + layoutModel.containerMarginLeft + layoutModel.containerMarginRight
        let cellHeight = containerSize.height + layoutModel.containerMarginTop + layoutModel.containerMarginBottom
        
        let cellSize = CGSize(width: cellWidth, height: cellHeight)
        
        cache[itemID] = NSValue(cgSize: cellSize)
...
//        }
        ...
        if layoutModel.staticContainerSize.equalTo(.zero) {
            ...
            let maximumWidth: CGFloat = itemWidth * 0.8
            assert(maximumWidth >= 0.0, "Maximum width cannot be a negative nuber. Please check your maxWidthMarginSpace value.")
            
            let dynamicSize = chatDelegate.collectionView(chatCollectionView, dynamicSizeAt: indexPath, maxWidth: maximumWidth)
            
            let finalWidth = dynamicSize.width + layoutModel.messageMarginLeft + layoutModel.messageMarginRight
            
            let cellHeight = dynamicSize.height + layoutModel.messageMarginTop + layoutModel.messageMarginBottom
            let finalCellHeight = max(cellHeight, layoutModel.avatarSize.height)
            
//            let minWidth = chatDelegate.collectionView(chatCollectionView, minWidthAt: indexPath)
//
//            finalSize = CGSize(width: min(max(finalWidth, minWidth), maximumWidth), height: finalCellHeight)
            finalSize = CGSize(width: min(finalWidth, maximumWidth), height: finalCellHeight)
        }
